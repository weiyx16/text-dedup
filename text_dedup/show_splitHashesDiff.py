"""
spark-submit --executor-memory=50G --driver-memory=4G --deploy-mode=client --executor-cores=64 hashonly.py   --data_path "/tmp/code/tiny_owt_split1"   --output "/tmp/code/tiny_owt_split1_hashes"   --column "text"   --ngram 13   --num_perm 10   --threshold 0.7
spark-submit --executor-memory=50G --driver-memory=4G --deploy-mode=client --executor-cores=64 hashonly.py   --data_path "/tmp/code/tiny_owt_split2"   --output "/tmp/code/tiny_owt_split2_hashes"   --column "text"   --ngram 13   --num_perm 10   --threshold 0.7
spark-submit --executor-memory=50G --driver-memory=1G --deploy-mode=client --executor-cores=64 show_hash_diff.py   --data_path_file "data_paths_debug.txt"   --output "/tmp/code/dedup_ids_debug"   --column "text"   --ngram 13   --num_perm 10   --threshold 0.7
what's in data_paths.txt:
    - /tmp/code/tiny_owt_split1
    - /tmp/code/tiny_owt_split2

in /tmp/code/tiny_owt_split1: openwebtext.1.txt
in /tmp/code/tiny_owt_split2: openwebtext.2.txt
"""
import hashlib
import re
import os
...
if __name__ == "__main__":
    ...
    for data_path in tqdm(data_paths):
        ...
        print("each items found: ", _records.count())
        if records:
    ...
    # merged hashes
    merged_records = load_hash("file://{}".format('/tmp/code/tiny_owt_hashes'))
    print("total items found (original one): ", merged_records.count())
    time.sleep(10)

    changes = merged_records.subtract(records).union(records.subtract(merged_records)).collect()
    print("Differences: ", len(changes))
    time.sleep(10)


    ## Further test on deduplication
    edges = create_edges(merged_records)
